...
--    Zapytanie ogranicz do konta nr '11-11111111' 
	...
	SELECT	kwota
		...
		,COUNT(*) OVER (ORDER BY data RANGE BETWEEN INTERVAL '6' MONTH PRECEDING AND INTERVAL '6' MONTH FOLLOWING) AS count6_6 
	FROM	transakcje 
	WHERE	nr_konta = '11-11111111' 
	ORDER BY data; 
...
--     gets) konieczną do wykonania obu wariantów rozwiązań dla tego samego zadania. 
	...
	-- zapytanie alternatywne dla zapytania z zadania pierwszego
	SELECT	* 
...
----------------------------------------------------------------------------------------------------------
	...
	--plan wykonania zapytania dla alternatywnego zapytania, bez funkcji analitycznych
